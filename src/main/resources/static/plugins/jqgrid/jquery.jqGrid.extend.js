...
//					setParam({postData: {parentId: postData.nodeid}});
		...
		loadError: function(data){
			...
			if (data.responseText && data.responseText != ''){
				bet.showErrorMessage(('操作失败，' + data.responseText + '！'));
			}
			bet.closeLoading();
		},
				...
				if (clickBinded == undefined){
					...
					$(this).click(function(){
						...
						if(confirm != undefined){
							bet.confirm(confirm, url, function(data){
								bet.showMessage(data.message);
								if(data.success== true){
									...
									if (confirmSuccess != undefined){
										try{
											...
											bet.print('confirmSuccess error: ' + e);
										}
									}else{
										// 如果是树结构表格
										if (options.treeGrid){
							...
							bet.prompt(prompt, url, function(data) {
								bet.showMessage(data.message);
								if(data.success){
									// 如果是树结构表格
									if (options.treeGrid){
							...
							if(type != undefined && type == "layer") {
								...
								bet.addTabPage(title, url, options.tabPageId)
							}
						}
						return false;
					});
	...
	function expandTreeNode(level){
		...
		setTimeout(function(){
			...
			if (dataGrid.expandNodeIds.length > 100){
				bet.showMessage("本次需要展开“" + dataGrid.expandNodeIds.length
						+ "”个节点，因为展开节点过多，执行取消。");
				dataGrid.expandNodeIds = [];
			}else{
				$('#'+dataGrid.expandNodeIds.shift()+':visible .tree-plus', dataGrid).click();
